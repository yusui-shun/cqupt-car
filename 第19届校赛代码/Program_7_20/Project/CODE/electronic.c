...
int16 ADC_DATA_CH0[6];   // 左横采样数组
int16 ADC_DATA_CH5[6];   // 右横采样数组

int16 ADC_DATA_CH2[6];   // 中横采样数组

int16 ADC_DATA_CH1[6];   // 左竖采样数组
int16 ADC_DATA_CH4[6];   // 右竖采样数组
...
{
	  adc_data_convert();
	  LnowADC[0] =  adc_filter(ADC_DATA_CH0);    // 左横
	  LnowADC[5] =  adc_filter(ADC_DATA_CH5);    // 右横
		LnowADC[2] =  adc_filter(ADC_DATA_CH2);    // 中横
	
		LnowADC[1] =  adc_filter(ADC_DATA_CH1);    // 左竖
	  LnowADC[4] =  adc_filter(ADC_DATA_CH4);    // 右竖
	
    MagneticField = LnowADC[0] + LnowADC[5];//磁场整体强度
		electro_error =
...
void adc_data_convert(void)
{
	...
	for(i=0; i<6; i++)
	{
		ADC_DATA_CH0[i] = adc_once(ADC0,ADC_10BIT);      // 左横
		ADC_DATA_CH5[i] = adc_once(ADC5,ADC_10BIT);      // 右横

		ADC_DATA_CH2[i] = adc_once(ADC2,ADC_10BIT);      // 中横
//		ADC_DATA_CH3[i] = adc_once(ADC3,ADC_10BIT);      // 右斜
		
		ADC_DATA_CH1[i] = adc_once(ADC1,ADC_10BIT);      // 左竖
		ADC_DATA_CH4[i] = adc_once(ADC4,ADC_10BIT);      // 右竖
	}
}
